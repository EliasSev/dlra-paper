"""
Matrix-free randomized SVD of the forward operator K.

OUT OF SCOPE for the paper (see CLAUDE.md): the rSVD *construction* of K is not
the contribution -- K and its factors U, S, Vt are treated as given inputs to the
solvers. This module is kept only because it is what produces those factors in
code. Do not build new solver work on top of it.
